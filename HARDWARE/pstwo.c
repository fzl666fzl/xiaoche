...
void PS2Control_Set(void)
{
	usb_ps2_ready = 1;
}

void PS2Control_Reset(void)
{
	usb_ps2_ready = 0;
	PS2_LX = 128;
	PS2_LY = 128;
...
void USBH_HID_EventCallback(USBH_HandleTypeDef *phost)
{
	...
	if( usb_ps2_ready==1 )
	{
		...
		PS2_RX = ps2->RX;
		PS2_RY = ps2->RY;
		if(ps2->getKeyState(PS2KEY_L2)) PS2_KEY=9;
	...
	else
	{
		...
		{
			PS2_ON_Flag=0;
			Move_X=0,Move_Y=0,Move_Z=0;//控制时热插拔,清空目标速度
		}
		PS2_RX = 128;
		PS2_RY = 128;
		PS2_LX = 128;
...
////////////////////// 注：以下标准PS2接口函数已弃用,目前使用usb接口PS2函数 ////////////////////////////////////
...
void PS2_Read(void)
{
  ...
  //读取右边遥感Y轴方向的模拟量  
	...
	//如果PS2手柄没有接入时摇杆的数据为0 0 0 0，读取的按键值为1，利用这个特性定义为手柄没接入，小车不能行动
